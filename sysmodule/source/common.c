...
static void listenThreadFun(void *arg) {
    ...
    while (exiting) {
        ...
        if (R_SUCCEEDED(rc) && (l.buttons != 0 || r.buttons != 0)) {
            if (playerEnable() && ((l.buttons | r.buttons) & getPlayLatestButtonMask()) == getPlayLatestButtonMask() && !playing && !recording) {
                log_info("macro matched: %s", macro_path);
                // 执行宏
                start_play(LATEST_MACRO_PATH, &playing);
                // 间隔 1000ms
                svcSleepThread(1000 * 1000000ULL);
            } else if (playerEnable() && maskMatch(l.buttons|r.buttons, &macro_path) && macro_path != NULL && macro_path[0] != '\0' && !playing && !recording) {
                log_info("macro matched: %s", macro_path);
                // 执行宏
                start_play(macro_path, &playing);
                // 间隔 1000ms
                svcSleepThread(1000 * 1000000ULL);
            } else if (recorderEnable() && ((l.buttons | r.buttons) & getRecordButtonMask()) == getRecordButtonMask() && !playing && !recording) {
                log_info("toggling recording");
                // 切换录制状态
                recorder_switch(&recording);
                // 间隔 1000ms
                svcSleepThread(1000 * 1000000ULL);
            }
        }
